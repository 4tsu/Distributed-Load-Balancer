...
// #include "2d/md.hpp"
```
インクルードで2次元と3次元を切り替え。無印は3次元。

```
md->set_params([ステップ数], [粒子位置ダンプ間隔(step)], [時間刻み]);
md->set_box([粒子数], [シミュレーションボックスのx長さ], [シミュレーションボックスのy長さ]);
#3次元 : md->set_box([粒子数], [シミュレーションボックスのx長さ], [シミュレーションボックスのy長さ], [シミュレーションボックスのy長さ]);
md->set_cutoff([カットオフ距離]);
...
粒子配置の読込ファイルについては、LAMMPSの`.dump`ファイルのフォーマットに準拠し、`x, y, z, vx, vy, vz`を入力する形となる。`sample/sample.dump`がお手本となる。ファイルを読み込むと、`set_box`で指定した数字などは上書きされる（カットオフ距離は除く）。ファイルにシミュレーションのステップ数を指定すると、そこからステップ数をカウントしてプログラムを開始する。これを利用して、シミュレーションのチェックポイント（バックアップデータ）を保存できる。デフォルトでは、`.ckpt`というファイルがチェックポイントとなっており、これを読み込むことで、シミュレーションを再開することが可能。また、3次元のシミュレーション結果から、断面を切り出して2次元粒子配置として読み込ませるコードが`sample/ttt.py`にある。このあたり、詳しくは[サンプル](#サンプルsample)を参照。

一方で、ファイル名を指定しない場合は、`set_config("make");`とすることで、粒子を一様に配置した系を生成する。

ちなみに、「`sdd`」はMPI並列をする際の空間分割の方法を表す。空間分割の仕方を工夫する事によって、各プロセスの計算負荷を均等にする事ができる（＝ロードバランサー）。番号によってロードバランサーの種類を指定する。
...
|3|RCB|Recursive Coordinate Bisectioning。最初にルートプロセスに全粒子を所属させ、各プロセスに順番に、もっとも多い粒子数のプロセスから半分を入れる。|
|4|1D Parallel| 薄い領域を一次元方向に並べ、壁の位置を両隣のプロセスと調整する分散処理型ロードバランサー。|
|5|Skew Boundary| 上記1D Parallelのように領域を一次元方向に並べるが、あらかじめシミュレーションボックスを細切りにしておき、各列の右端と一つずらした左端をつなげるSkew Boundaryを課すことで、分割された領域が薄くならないように工夫したもの。|

また、`MD *md = new MD(mi);`から`delete md;`までをループで回し、`md->run();`の引数にループカウンタを入れることで、ループの数だけ別の試行として（乱数とか挙動は変わらない）時間の計測データを取ることができる。なお、引数0は単発とみなされるので注意。
```
// 3回連続でシミュレーションを実行する
// 結果のファイル名にロードバランサー番号と試行回数が入る（例：evergy_0_3.dat）
for (int i=1; i<=3; i++) {
    md->run(i);
}

// 引数ゼロはファイル名にロードバランサー番号と試行回数が入らないので注意
md->run(0);
// ファイル名はenergy.datのようになる
```
...
- 運動エネルギーとポテンシャルエネルギー、および系の全エネルギーの変化（energy.dat）
- LAMMPSの`.dump`ファイル形式をしている、チェックポイント2つ（ckpt/.ckpt）

### 結果の可視化
- `.cdv`は[cdview][4]という可視化プログラムのフォーマットで出力。cdviewを使えば粒子の動きをインタラクティブに見ることができる。また、cdviewが使用できない環境向けに、`vis`フォルダ内の`vis.py`の実行で、`vis`にgifアニメーションが生成される（`cdview.gif`）。
...
をコメントアウトし、`make`。
#### 実行
実行前に前回の結果出力は削除しておくのが安全だ。とくに、エネルギー出力ファイルは前回のシミュレーション結果への追記になってしまうので、必ず削除しておく。
```
rm *.cdv
rm energy.dat
mpirun -np 並列数 ./md.exe
...
また、一時ファイル`.temp`の自動削除が行われないため、大量にファイルが残る。これらは上書き方式であるため、残したまま次のシミュレーションを行っても問題ないはずだが、邪魔だし、シミュレーション完了時点では不要なので消すことを推奨する。
...
- sample.dump : `.dump`ファイルのサンプル。本プログラムは、この形式の粒子配置ファイルを読み込むことができる。自分で記述するときは、`ITEM: TIMESTEP`の値を0にしてはいけない。ここが0だと、自動で飛ばすように組んでいる（LAMMPSの続きからやるときに便利）。`ITEM: TIMESTEP`は書かなくても読み込まれ、その場合は、step 0からシミュレーションがスタートする。
...
`makeconf.py` -> `run.in` -> `ttt.py` の順に実行すると、デフォルトでは`sample2d.dump`がsampleフォルダ内に生成される。これを`md.exe`と同じ場所に移動し、`set_config()`の引数にファイル名を指定することで、液滴のサンプルシミュレーションが動く。
```
cd sample
python3 makeconf.py
